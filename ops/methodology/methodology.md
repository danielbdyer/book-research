...
- [[sharp questions]] — the standard for research questions, issued 2026-08-06 under the author's license to sharpen the apparatus: a question is sharp when it can fail, when its stake is named, when its address is page-level, when it states what it inherits from completed readings, and when its current evidence is graded. A name plus a topic gloss is a shelf entry, not work, and the two are never mixed without the difference visible.
...
- [[the chain of understanding]] — the author's directive of 2026-08-08 on the research's main goal: development and mapping of ideas that builds the chain of belief and understanding needed for sensible thinking about the book's overarching opportunities. The deliverable is the chain, not coverage; a proposed piece of research names the link it develops or the gap it maps, and work that cannot name its link waits on the shelf.
- [[the clearing]] — the policy for the clearing register (`ops/clearing.md`), added 2026-08-09: a place that holds what the center of gravity structurally cannot see, admitted under an inverted rule — a thing enters because it does not yet fit, and is protected from the writing test, its one discipline being to name which commitment of the center cannot hold it.

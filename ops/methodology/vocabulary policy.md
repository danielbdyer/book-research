...
1. **The reference and research surface is denotative.** Filenames, YAML field names, enum values, folder names, and primary headings name the concern in plain terms: `interlocutors`, `reference-lineage`, `tensions`, `book-structure`, `research-frontier`, `relational-field`, `the call and the answering`, `container-anatomy`.

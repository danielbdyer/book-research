...
This file treats the corpus's two outlines — the atlas's section structure and the mythic operating system edition's contents — as research scaffolding. It records what territory the project has named and how far each name has been explored.

**What this is not.** It is not a proposed structure for the book and carries no weight as one. An entry here is a gesture: a territory the author's own documents named, at some point, in some arrangement. Naming a territory is evidence that the project has been to its edge, not evidence that it belongs in the manuscript, and nothing here should be read as a chapter, a part, or a commitment. The reference directive applies throughout (`ops/methodology/reference by name.md`): units are named, and any number is a citation locator into one document.
...
|---|---|---|---|
| §0 | How to read the map; the notation set | The notation is recorded in [[figure glossary]] | — |
...
## The mythic operating system contents as twenty-eight gestures

The earliest edition names twenty-eight units in six parts. One is written; the rest exist as names with a subtitle each. Read as scaffolding rather than as a plan, the list is the widest statement of scope the project has ever made, and most of it has never been taken anywhere.

The state column distinguishes three conditions. **Written** means prose exists. **Carried** means the territory reappears in the arc under another name and the vault holds material on it. **Open** means the name exists and nothing else does.
...
| Gesture | State | Note |
|---|---|---|
| Relational Spectrums and Complementarity: conflict, contrast, completion | carried | Adjacent to polarity in Erotic-Spiritual Intelligence |
...
The file is maintained by re-reading the two outlines when either document is revised, and by moving entries between states as extraction proceeds. It does not grow by adding territory the corpus has not named — that is the outward frontier's job, and it belongs in [[research-frontier]].

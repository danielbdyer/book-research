...
2. **The molecule reconciliation.** The only element where two documents disagree about content rather than naming or scope. The extraction half is done: `ops/formula audit.md` holds every formula written with an operator across all four sources, twenty-one in all, with a per-source table. What remains is the membership reconciliation — which formulas each catalogue contains and which appear in only one.

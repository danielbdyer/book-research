...
- The book's ending **differs between** the descriptive sources and the treatment on one side and the executed draft on the other, found by the second-person survey of 2026-09-03 (`ops/second person ecology.md`, tension 3) and re-located against the extracted corpus the same day: `first-telling` (p. 12, "The final words sit at the bottom of that breath: welcome home"; the answerability at p. 13, "the last line of the book is addressed to the reader, and it's written so that it can be answered"), `walked` (p. 23, "Its last gesture: Welcome home. And the book ends… answerable"), `atlas §X.3` (p. 24, "'Welcome home' · the book ends answerable"), and `arc-gloss` (overlay three, p. 3, "rest at the bottom (Epilogue: 'Welcome home')"; overlay two, p. 3, "It ends answerable"; and the Epilogue's storyboard, p. 16, "Last words already found: … you've always been ready; Welcome home," with the Jewel at p. 17, "'Welcome home' as the final two words of a book whose first image was a house") describe a last line written so that it can be answered and ending on "welcome home," while `abridged` closes its Epilogue "The clearing was always there. You have always been ready. Step in." (p. 47) — the Prologue's own closing lines with one added (p. 3, "The clearing was always there. Step in.") — and ends, after the Note on Sources, "For now, what you have is enough. Step in. — end —" (p. 49); the phrase "welcome home" does not occur in it, and `mythic-os` has "Welcome home." at the close of its first chapter (p. 21) rather than at the end of the book. A difference between two states of one element, filed here as a row; it becomes a tension, a disagreement inside the book's doctrine kept open in a note rather than resolved, only if the author holds the answerable ending as doctrine.
- The arc's units **differ between** the descriptive sources and the drafts, found by the arc inquiry of 2026-09-03 (`ops/the arc as kindness.md`) and re-counted against the extracted corpus the same day: the six phase names — preparation, onset, deepening, peak, descent, integration — appear as a list only at `atlas §X.1` (p. 22), `walked` (p. 21), and `first-telling` (p. 3), and no draft carries the list; `arc-gloss` and `abridged` carry five named parts, and `mythic-os` six themed parts with no peak. The drafts use four of the six words, as the vocabulary of the psychedelic experience the Break chapters describe rather than as names for the reader's arc — "After the peak, the descent" (`abridged` p. 35), "peak experiences without preparation and integration are risky" (p. 38) — and the treatment glosses one part with two of them, Part IV "(peak + descent; prose destabilizes on purpose)" (`arc-gloss` p. 12). The word "onset" occurs nowhere else in the corpus, and "deepening" once more only as a participle, "each layer deepening the reader's relationship" (`mythic-os` p. 1). A difference between two states of one element, filed here as a row.

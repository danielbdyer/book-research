...
**`notes/the verification standard rests on one name and that name has no chair.md`** carries the assertion in four places: the title, the `description`, the statement layer ("And Gendlin has no interlocutor note in this vault"), and a substantiation bullet that reports a check by name — "`rg -l 'Gendlin' notes/` returns five claim notes plus the interlocutors map, and no note carries `category: interlocutor` for him." Re-run today that command returns nineteen files, and `notes/eugene gendlin.md` carries `category: interlocutor`. The note's own footer already contradicts its body: "[[eugene gendlin]] — the chair, written 2026-08-05."
...
The narrow repair is separate from all three and is not a session's to make, because the first note's title carries the expired clause and fourteen files link that title, including `ops/orientation.md` and `ops/methodology/thesis-bearing and company.md`. Retitling is a /refactor with a stated plan. Correcting the three bodies to the dated form is smaller, and is the shape the vault has already chosen once.

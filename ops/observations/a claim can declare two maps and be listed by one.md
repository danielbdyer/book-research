...
## The counted condition now exists, 2026-08-05

Answered by the /reflect pass that closed the 65 map-list gaps. It is the third check in `scripts/queries/unconnected-claims.sh`, and it is narrower than this note proposed: it counts a note that appears in **no** declared map, which is the schema floor CLAUDE.md states, and not a note listed by one declared map and missing from another. The measurement that decided the scope is the one this note anticipated — the every-declared-map count fired 65 times where the floor fired 11 — so the wider version would have reproduced the false-positive problem this note warned about from the other direction. The section-structure exemption this note identified is built in: the query treats a link anywhere in the map's body as membership, so a claim carried in the map's prose outside its claim list is not reported. The decisions row is in `ops/decisions.md`.

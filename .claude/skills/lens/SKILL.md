...
2. **Reconcile the family against the graph and the queue (the sync pass).** The 2026-08-21 harvest showed this was the missing step: families drift out of sync with the notes and the reading queue because a family is authored from a snapshot and the graph keeps moving. For each family, three checks:
   - **Promote** — if a new note is an *attested member* of one of the family's open edges (a candidate the graph has now filled), move it from the open edge to the attested members and link it. This is the substrate loop closing: the frontier told the reading queue what to find, and the note comes back as a member. Promotion is admission — a judgment, not a batch move.
   - **Correct drift** — check the family's "named-but-unseated" / "not yet seated" assertions against the actual notes. A thinker the family calls unseated who *now has a note* is a factual error (the family says unseated; the graph says seated) — wikilink it and move it from the open edge to attested. `grep -lE '^category: interlocutor' notes/*.md` is the roster to check against.
   - **De-duplicate the frontier and catch the overturned** — cross-check the family's open-edge reading-questions against `ops/reading queue.md` (the tiered backlog, its Done section, and the epistemic-audit / second-pass deferrals). An open edge the queue already tracks should *point to that entry*, not re-propose the read; an open edge the queue has already *answered* (a Done reading, with a record in `archive/`) should be folded in as an attested finding with its kept refusal; and — the highest-value case — a family claim the reading record has *qualified or overturned* (the Kyoto-School pattern: a convergence stated as clean independence that a reading found to be company-with-a-refusal) must be corrected to state the finding *with its refusal intact*. This stays via positiva: a convergence held with its refusal is the more alive version, never a debunking.

...
**The shape of a family file.** Every family is authored from `the-conversation-catalog.md` (the deduplicated catalog of the reframe conversation's structures) and the primary transcript, deepened with the notes the graph already holds. A family finds *its own* organizing frame from the source material — the Loci is a register of standpoints, the Axioms is a three-layer architecture — so do not force one template; find the structure the material already has. What every family shares: each member carries where it came from (a transcript locator) and a **provenance** tag (*author-voiced* = primary, from a `## Prompt:` turn; *model-produced* = works-consulted, from a `## Response:` turn; *mixed*); each member is either **attested** (installed as a `[[note]]`, linked) or **candidate** (only in the transcript so far); and each region names its **open edge** — the members implied but not yet named, which is the frontier the fourth question feeds.

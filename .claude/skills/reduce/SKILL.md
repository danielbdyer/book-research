...
| Tension: content contradicts existing {vocabulary.note} | If the contradiction is in the BOOK'S doctrine: a claim in notes/ with category `tension`, status and treatment fields, via the pipeline. Only a contradiction in the VAULT'S OWN process goes to `ops/tensions/` (CLAUDE.md: the two are never mixed) |

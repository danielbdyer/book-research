...
Read `ops/derivation.md` (the coherence-check section records the constraint reasoning; the upstream plugin's reference/interaction-constraints.md applies when the plugin is installed) and check:
...
2. Consult the upstream plugin's reference/ directory if the plugin is installed; otherwise follow the pattern of the existing skills in .claude/skills/

...
- **A note reads wrong.** Check it against the standing directives in `ops/methodology/` — start with denotative naming, the prose register, and the register countermeasure's seven lines — and the templates' `_schema` blocks. The most common faults: a description that restates the title, an unmarked quotation, a mechanism reference that does not name its file, and a narrative term used as if it were a schema term.
- **A session start or a turn prints "REGISTER COUNTERMEASURE: a surface above has drifted."** A stamped copy of the register countermeasure no longer matches the canonical block, or a hook wire is cut. Restore the drifted surface from the final section of `ops/methodology/prose register.md`, and move every copy plus the signature and ban lists in `scripts/queries/countermeasure.sh` in one commit — the check fails loudly until every copy has moved, which is the amendment protocol working rather than a bug.

...
5. **Verification.** The vault verifies structure, provenance, and coherence. It cannot run the book's own somatic test; that certification belongs to the author. The somatic test has documented limits, and one of them applies to the vault directly: a well-formed note can read as true because it is well-formed.
...
- [[the book's privative grammar runs four different engines under one word]] — a rule for imports: one word carries four engines, so a practice-model entering the graph is asked which engine it runs before it is filed under the doctrine's word

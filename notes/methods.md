...
One is cited widely enough to state its form here. The **provenance reconstruction** (2026-08-02) is an account of how the corpus was made, commissioned by the author from an agent with access to their conversation threads, and held outside this repository. It is cited as `provenance reconstruction §1` where it answers one of the request's numbered questions and by section name where it does not; quotations from the record it retrieves carry the speaker mark and date it supplies — `provenance reconstruction §1, quoting [D] 7 Jul 2026`. It was considered for a source standing and routed here instead; the reasoning is in `ops/methodology/source standing.md`.

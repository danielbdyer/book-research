...
The book promises that what it describes is livable inside an ordinary week — a Tuesday, a job, a family — rather than requiring withdrawal ([[the tuesday is the temple]]). The claim carries real weight, because it is what separates the book from the retreat literature its reader has already tried, and its strongest support is that the tradition beneath the book solved the renunciation problem a thousand years ago: Trika is a householder tradition, and that is one of the four things the mesh grid says makes it irreplaceable.

The support is also the problem. The claim currently rests entirely inside the lineage the book descends from, so agreement about it is a property of that lineage rather than an arrival at it. The sort ranks the gap fifth of nine and states it flatly: "The Householder Axiom is a load-bearing claim with zero independent witnesses" (`mesh grid: the search map`).

Three candidates are named, all contemplative and none Indic. *Avodah be-gashmiyut*, worship through corporeality, in the Hasidic tradition — the same claim under another God in another hemisphere. Brother Lawrence's kitchen. And Ignatian contemplation in action, which is already half-present in the vault.

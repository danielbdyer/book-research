...
- [[karla mclaren]] — emotion-work. Channeling, the middle path she coined, is the practice the book runs beneath its forms; the grief-witness clause the corpus gives her is the book's hardening of her ritual, read at her text.
...
- [[rainer maria rilke]] — first chapter-patron by the corpus's roster, the note built because the round's claims needed it; the torso poem performs the book's event out of a trained practice of looking. Two refusals are kept: the devils priced with the angels, and art reachable only by love.

The mesh grid's names, evaluated 2026-08-05 from the grid alone rather than against primary works. Each note says so; every one carries a reading still to be done, and none of the loads is a seating.
...
- [[the title's idiom names the integration of the three loops rather than parallel lines of development]] — the title's premise re-assigned: the executed prose defines it as the three loops integrated, and Wilber, read, denies the all-lines ideal, so his lines are a precedent for the engine's question and not the title's author
- [[fractured development is a third account of why the big experience did not hold]] — Wilber's stone Buddha, read at his texts, a third account of the failed breakthrough that gives the chair a mechanism the grid did not list
...
- [[al-qushayri]] — independent-arrival. States that descend against stations that are earned, a millennium before the lattice; read at Knysh's translation, which shares the lattice's non-transfer rule and not its two-axis geometry.
...
- **chapter-patron (in chapter order, per the arc-gloss second edition, which assigns all sixteen).** Rilke (front matter), Alexander, Buber, Heraclitus, Heraclitus, Gendlin with McLaren, Meadows, McGilchrist, Buber with hooks, Whyte, Kimmerer, Rumi, Eliot, Tippett, brown, Meadows; atlas §IX.9 differs at two seats — Lao Tzu for the Generative Triad and hooks alone for the Third Entity — recorded as two rows in `ops/chapter concordance.md`. Meadows is the only name below the front row with two seats and has no chair ([[the routing surfaces carry four more names alone than the sort filed as company]]).

...
5. The backlog splits into a work tier and a shelf. The work tier is `ops/reading queue.md` — nineteen readings, each with a question that can fail, its stake, and a page-level address. The pending lists in `ops/queue/` are the shelf behind it, and the author had that shelf cut on 2026-08-07 from ninety-six unevaluated names to twenty, each survivor stating the live vault question it bears on and each of the sixty-one declines carrying its reason in `ops/queue/declines.md`. The conditions a piece of writing needs before drafting are stated at `ops/manuscript readiness.md`. The user manual is at `manual/manual.md`.

...
**Sheer emotion.** The felt sense is internally complex; an emotion is "internally all one quality… they are 'sheer.'" Feeling ashamed is not the same as sensing what the shame is about, and the emotions "often keep him from sensing that in himself which is the complex ground of the emotion." His observation: "we feel guilt, shame, and badness, instead of feeling that concerning which we feel shame, guilt, and badness."

**The circumstantial orbit.** Replaying what happened, what one should have done, what others did — "circumstantial play and replay, the inward repetitions of conversations, and dramatic re-enactings," which can fill sleepless nights without once touching the felt meaning of "all this."

**The explanatory orbit.** Self-interpretation, and the footnote is explicit that accuracy does not redeem it: "Whether the explanatory concepts are simple and foolish, or sophisticated and quite correct, they are useless unless one employs them as pointers to momentarily name and hold onto a directly felt meaning… even when they are accurate they are helpless abstractions."

**Self-engineering.** Talking at oneself, "arranging and rearranging one's feelings without stopping to sense quite what they are." The footnote then makes a claim the vault did not expect to find here: willpower is real and often necessary, but "they cannot be effectively exerted at points where experiencing does not implicitly function." Will works exactly where the process is already carrying forward, and fails where it is not. The clause has a near-twin from the opposite side of the fork, arrived at independently and read 2026-09-02: Tony Robbins, the will pole's own holder, holds that "Willpower by itself is not enough—not if we want to achieve lasting change" and that "overriding our pain in the short term by pure willpower never lasts" (*Awaken the Giant Within*, chs. 5 and 3), and defines his own method as the thing that makes willpower unnecessary — conditioning that lets a person succeed "without constant effort or willpower." The voluntarist agrees that raw volition fails and differs from Gendlin only on what succeeds instead, conditioning rather than carrying forward, which is company for the clause rather than a defeat of the pole ([[tony robbins]]).

## Substantiation

- The four are distinguished from focusing by one operational mark, stated at def. 11 of the same paper: none of them produces referent movement. "From this lack of referent movement, one knows that nothing has really changed." The counterfeit-detection is therefore not a matter of judging the quality of one's attention; it is read off the same signal the standard already uses.
...
- One taxonomic care: the footnote's subject is attention that fails to *be* focusing, and the corpus's counterweight is prose that *simulates* the shift. Those are different failure directions — one produces no signature, the other forges one — and this claim covers the first. The forgery case remains a separate problem, and its counterweights are still unread.

...
There is a difference in grammar between a sentence that wants assent and a sentence that wants a reply, and the book's promised event turns on it — the last line is written so that it *can* be answered, a design decision made at the level of syntax, not sentiment. A declarative that seeks belief finishes itself and leaves the reader nothing to do but nod or refuse; an answerable sentence — the question, the second-person address, the line left open at its end — leaves a socket where the reader's move fits. What the reader gains is room: the answerable sentence does not enclose them in a completed proposition but keeps a place open for their own next word, so reading becomes something they are *inside of* rather than something done to them. Its use to the writing is a concrete craft handle below chapter and image — does this sentence leave a socket for the reader's move?
...
Relevant Notes:
- [[the poems end by naming the next act and leaving it undone]] — the answerable ending already in the primary layer, the line left open for the reader's move
- [[the book asks only for attention until it has given the reader something to act with]] — the sentence-level socket serving the same offer, room left for the reader to act
- [[double reflection withholds the result so the reader must reproduce it in his own existence]] — the neighboring device, withholding the result so the reader supplies it

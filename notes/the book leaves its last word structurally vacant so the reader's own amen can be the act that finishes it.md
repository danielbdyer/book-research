...
Liturgy has a device the book seems already to want: the "Amen," the assembly's answer that does not comment on the prayer but *completes* it — the thanksgiving is unfinished until the people say the word. If the book's last line is written so that it can be answered, then the book is doing in solitude what liturgy does in company: keeping the final position deliberately empty, reserving the ratifying word for the one addressed. What the reader gains is the last word — not a summary handed to them but a vacancy shaped exactly for their reply, so the thing they say (aloud or in their turning) is not a footnote to the book but its consummation. Its use to the writing is to name a concrete architectural device — the deliberately vacant terminal position — and give it a lineage that explains why leaving the last word to the reader completes rather than abandons the work.
...
Relevant Notes:
- [[the poems end by naming the next act and leaving it undone]] — the vacant terminal position already in the primary layer, the next act reserved for the reader
- [[the poem is a letter cast toward an addressee it trusts exists so the reader is the over-against it was reaching for]] — the reach this is read against, the letter-answered-alone perhaps truer than the Amen-in-company
- [[the book's meaning is completed in the reader's answer so the reply is the writing's final movement]] — the completion this gives an architectural device, the last word reserved

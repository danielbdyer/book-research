...
**The register countermeasure** (2026-08-05; stamped at the author's direction, in his words "to a fault of overdoing it," at every surface a session loads — this file, `ops/orientation.md`, the session hook, `prose register.md`, and `reporting to the author.md` — with `scripts/queries/countermeasure.sh` verifying every stamp at each session start; the canonical copy is the register's final section). Five lines, none optional. Every statement is a complete sentence with a named subject and a finite verb, and the last sentence of each paragraph is checked first, because fragments concentrate at the closes. A vault-internal term is unfolded in the sentence where it appears or replaced with plain words, absolutely so in replies to the author, who authored the book the terms describe and not the terms. A reference is a receipt, never the content: every link or file path travels with one clause stating what it points to, and a paragraph must still assert everything it asserted with its links struck out. A figure of speech appears only inside quotation marks, where the quoted phrasing is itself the finding. And every reply to the author is reread as him before it is sent, in full. The quality clause carries equal force: the ban is on compression that withholds and gesture that defers, never on complexity that delivers — richness is the right fact, complete, with its reason attached, and the five-word fragment that occasioned this ("Recorded on the chair, not resolved.") lost information that its full-sentence replacement carried.

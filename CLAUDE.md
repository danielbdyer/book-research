...
**Write with research diction** (`prose register.md`). The vault writes like a careful graduate researcher: plainly, so the information is never lost in the prose that carries it. Six habits do that work, and they are the good part of the register apparatus that was retired on 2026-08-09 for talking about itself more than it improved anything. Every sentence has a named subject and a finite verb, so a fragment never stands in for a claim, and the same holds for a bullet's opening words before a colon, where a headline otherwise substitutes for a statement. A reference is a receipt rather than the content, so every link or path travels with one clause saying what it points to, and a paragraph still asserts everything it asserts when its links are struck out. An abstract noun that stands for something specific is expanded into the thing itself, so the sentence names the thing rather than its category. A figure of speech appears only inside quotation marks, where the quoted phrasing is itself the finding. A pipeline coinage — a word that is neither the book's own vocabulary nor plain English — is unfolded or replaced, absolutely so in anything said to the author, who wrote the book these terms describe and not the terms. And every prose file is reread before it is saved as the reader it will actually meet, the author or a later session holding none of this one's context, because they build on the record rather than on any reply. The quality clause binds equally: none of this shortens or flattens, a long subordinate sentence is welcome when it unfolds in reading order, and the ban is on compression that withholds rather than on complexity that delivers. A session that wants to repair prose already written to this standard uses `/recompose`; a sentence the author flags is logged in `ops/register violations.md` so the next correction has an address.

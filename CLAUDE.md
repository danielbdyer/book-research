...
**The register countermeasure** (2026-08-05; stamped at the author's direction, in his words "to a fault of overdoing it," at every surface a session loads — this file, `ops/orientation.md`, the session hook, `prose register.md`, `reporting to the author.md`, and `ops/derivation-manifest.md`, which the pipeline skills read at invocation — with a per-turn hook printing the compact form before every reply, two write-time reporters — one naming the residue shapes of past violations in a file's prose before the write lands, one scanning shell-batched diffs with the same detector after every command, neither of them blocking since 2026-08-09 — and `scripts/queries/countermeasure.sh` verifying ten content signatures at every surface on each session start; the canonical copy is the register's final section, and sentences the author flags are logged in `ops/register violations.md` so the next correction has an address). The countermeasure has seven lines, and none is optional. Every statement is a complete sentence with a named subject and a finite verb, and the last sentence of each paragraph is checked first, because fragments concentrate at the closes, and in list-shaped prose the bullet's opening words before a colon are checked at the same priority, because there a headline substitutes for a claim. A vault-internal term — chair, seat, load, privated, and every other pipeline coinage — is unfolded in the sentence where it appears or replaced with plain words, absolutely so in replies to the author, who authored the book the terms describe and not the terms; the test is that a word neither the book's nor plain English is the pipeline's. A reference is a receipt, never the content: every link or file path travels with one clause stating what it points to, and a paragraph must still assert everything it asserted with its links struck out. A figure of speech appears only inside quotation marks, where the quoted phrasing is itself the finding. An abstract noun that stands for something specific is expanded where it stands — the sentence names the thing, not its category — with consequence-sentences checked first, because abstraction concentrates where meanings are stated. Every reply to the author is reread as him before it is sent, in full, and every prose file the session writes — a note, an archive record, an ops surface, a queue entry — is reread the same way before it is saved, as the reader it will actually meet: the author, or a later session holding none of this one's context; there is no internal register, because the author and the next agent build on the record, not on the reply. And a subagent's report is raw material, never copy: its findings are recomposed under these lines before they are filed, and every research prompt a session sends states the register's demands. The quality clause carries equal force: the ban is on compression that withholds and gesture that defers, never on complexity that delivers — richness is the right fact, complete, with its reason attached, and the five-word fragment that occasioned this ("Recorded on the chair, not resolved.") lost information that its full-sentence replacement carried.
...
- **Structural queries**: `rg` over YAML (for example `rg '^category: tension' notes/`), plus `scripts/queries/` — `unconnected-claims.sh`, `nascent-stubs.sh`, `coverage.sh` (citations per source, notes per category), `tensions-status.sh`, `candidate-seats.sh`, `placements.sh`, `clusters.sh` (the graph's regions, the instrument behind `ops/convergence points.md`), `lineage-sort.sh` (the discipline-by-load grid behind `ops/lineage sort.md`), `frontmatter-parse.sh` (notes, archive records, and observations checked against the templates' enums and cross-field rules), `strike-links.sh` (paragraphs thinnest after their links are struck, as cold-read candidates), `outline.sh` (the census half of `ops/outline.md`: the propositions the graph leans on hardest, the material by concern and by found region, and what is thin), and `countermeasure.sh` (the register stamp check).

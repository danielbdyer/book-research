...
**Write to the author under the same register, plus one addition** (`reporting to the author.md`). What a session says to him is held to the register above, because the assumption that he arrives with the context is only half true: he carries the book, and the vault's working vocabulary — chair, load, hole, seat, concordance row, treatment under trial — is the pipeline's invention rather than his. He authored the book those terms describe, not the terms. So a vault-internal term is given its referent before it is used as a handle; a finding is reported with what produced it and what was inferred rather than found; a proposal states the problem and one real instance of each option before its conclusion, because a proposal reported as its conclusion alone asks him to approve a sentence rather than a decision; and the strength of the evidence is stated with the recommendation, since two independent events and a single data point are different situations. Compression stays required — this is a demand for context, not volume. And a reply is reread as its reader before it is sent (rule 7, author correction 2026-08-05): every statement a complete sentence with a subject and a verb, every vault-internal term unfolded in place or dropped, and the last sentence of each paragraph checked first, because fragments concentrate at the closes — the correction's occasion was five words, "Recorded on the chair, not resolved."
